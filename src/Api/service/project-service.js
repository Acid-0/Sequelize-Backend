...
async function postProjectService(model, images) {
    if (model.id == 0 || !model.id) {
        ...
        let project_update_res = await project.update(model, { where: { id: model.id } });
        if (project_update_res[0] !== 1) return null;
        let images_to_delete = project_image.findAll({
            where: {
                project_id: model.id
            }
        });
        ...
        images_to_delete.forEach(img => {
            const path = "uploads/" + img.image_path
            fs.unlink(path, (err) => {
                if (err) {
                    console.log('image delete error:', path, err)
                }
            })
        });
        ...
        const project_image_res = await project_image.bulkCreate(imagesToStore);
        if (project_image_res) return project_update_res;
        else return null;
...
async function deleteProjectService(model) {
    if (model.id > 0) {
        ...
        images_to_delete.forEach(img => {
            ...
            fs.unlink(path, (err => console.log('image file not deleted')))
        });
